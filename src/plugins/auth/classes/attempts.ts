      ...
      export default class AuthAttempts {
            ...
            allowed(action: AuthActions, ip: string): boolean {
                  ...
                  if (attempts.length >= maxAttempts) {
                        ...
                        if ((timestamp.getTime() - firstAttemptTimestamp) <= lockoutTime) {
                              return false; // Max attempts exceeded within lockout time
                        }
                  }
            ...
            watchdog(): void {
                  this.attempts = this.attempts.filter(attempt => {
                        const timestamp = new Date();
                        // Keep only attempts that are within the defined lockout time interval
