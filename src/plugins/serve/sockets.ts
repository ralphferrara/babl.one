...
//||=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-||*/
      ...
      import ChirpRequestSocket                     from '../../classes/server/chirp.request.socket';
      import ChirpResponseSocket                    from '../../classes/server/chirp.response.socket';
      ...
      class SocketInstance {
            ...
            async start() {
                  ...
                  this.wss.on('connection', (socket: WebSocket, request: http.IncomingMessage) => {
                        ...
                        app.log(`WebSocket [${myUUID}] New connection`, 'info');
                        socket.uuid  = myUUID;
      ...
      export default class SocketPlugin {
            ...
            static async init(app: any) {
                  ...
                  const config    = await app.path('./config/sockets.json').json({});
