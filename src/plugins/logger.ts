/*||=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-||
//|| babl.one :: plugins/logger.ts
//|| Logger Plugin - Integrates styled terminal logging into App instance
//||=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-||*/
      ...
      export default function LoggerPlugin(app: App) {
            app.provide('logger', new Log());
      }
      ...
      class Log {
            ...
            log(message: string | string[], level: string = 'info', e?: any) {
                  new LogDispatcher(message, level, e);
            }
      ...
      class LogDispatcher {
            ...
            echo(message: string | string[], level: string, color: string) {
                  ...
                  for (const line of lines) {
                        console.log(this.color(color) + '['+level+'] - ' + new Date().toLocaleTimeString() + ' - ' + line + this.color('reset') + ' [PID:' + process.pid + ']');
                  }
            }
